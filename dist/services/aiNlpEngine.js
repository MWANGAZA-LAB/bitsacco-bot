...
User Context: You're chatting with Kenyan users via WhatsApp who want to save money and learn about Bitcoin.`;
     ...
     */
    async processMessage(userId, phoneNumber, message) {
        ...
        try {
            ...
            const processedResponse = await this.processAIResponse(userId, phoneNumber, aiResponse.data, intent);
    ...
    recognizeIntent(message) {
        ...
        for (const [intent, patterns] of Object.entries(this.intentPatterns)) {
            const matches = patterns.filter(pattern => lowerMessage.includes(pattern.toLowerCase())).length;
     ...
     */
    async handleAuthenticationFlow(userId, phoneNumber, message, intent) {
        try {
     ...
     */
    async prepareAIContext(userId, phoneNumber, intent) {
        const aiContext = {
            user: {
                id: userId,
                phone: phoneNumber,
     ...
     */
    async generateAIResponse(conversationContext, aiContext) {
        if (!this.isEnabled) {
     ...
     * Process AI response and execute actions
     */
    async processAIResponse(userId, phoneNumber, aiResponse, intent) {
        const response = {
        ...
        if (config.features?.voiceEnabled && response.text.length < 500) {
            try {
                const voiceResult = await voiceSynthesizer.synthesizeText(response.text);
     ...
     */
    async executeAction(userId, _phoneNumber, action, response) {
        try {
     ...
     */
    trimConversationContext(userId) {
...
//# sourceMappingURL=aiNlpEngine.js.map