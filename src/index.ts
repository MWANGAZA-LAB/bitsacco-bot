...
/**
 * Bitsacco WhatsApp Assistant Server
 * TypeScript version with enhanced type safety
 * Enterprise-grade Node.js application with comprehensive service architecture
...
interface ServicesStatus {
  [serviceName: string]: ServiceStats;
}
...
class TypedBitsaccoServer {
  ...
  private async registerRoutes(): Promise<void> {
    ...
    try {
      ...
      this.app.get('/webhook/whatsapp', async (request: FastifyRequest<{ Querystring: WebhookQuery }>, reply: FastifyReply) => {
        const { query } = request;
      ...
      this.app.post('/webhook/whatsapp', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
      ...
      this.app.get('/api/services/status', async (request: FastifyRequest, reply: FastifyReply) => {
        const services = await this.getServicesStatus();
        reply.send(services);
      });

      this.app.get('/api/services/:serviceName/stats', async (request: FastifyRequest<{ Params: { serviceName: string } }>, reply: FastifyReply) => {
      ...
      this.app.get('/api/info', async (request: FastifyRequest, reply: FastifyReply) => {
        reply.send({
   ...
   */
  private async getServiceStats(serviceName: string): Promise<any | null> {
   ...
   * Get server status
   */
  getStatus(): { isStarted: boolean; startTime: Date; uptime: number } {
    return {
...
// Start the server
typedServer.start().catch((error: any) => {
