...
/**
 * WhatsApp Bot Service Module for Bitsacco
 * TypeScript version with enhanced type safety
 * Handles WhatsApp client and message processing
 * 
...
interface UserSession {
  state: 'init' | 'awaiting_phone' | 'awaiting_otp' | 'authenticated' | 'awaiting_load_amount' | 'awaiting_load_method' | 'awaiting_withdraw_amount' | 'awaiting_withdraw_destination' | 'awaiting_withdraw_otp';
  phone?: string;
  bitsaccoUserId?: string;
  loadAmount?: number;
...
class TypedWhatsAppBotService {
  ...
  private maxConversationHistory: number = 10;
   ...
   */
  private async handleAuthenticatedCommands(userId: string, text: string, session: UserSession): Promise<void> {
   ...
   */
  private async processAIActions(userId: string, actions: any[], session: UserSession): Promise<void> {
   ...
   */
  private async sendBranding(userId: string, options: BrandingOptions = {}): Promise<void> {
   ...
   */
  private async promptPhone(userId: string): Promise<void> {
   ...
   * Send audio message
   */
  private async sendAudioMessage(to: string, audioFilePath: string): Promise<void> {
    try {
   ...
   */
  private createMessageContext(message: Message): MessageContext {
    return {
      userId: message.from,
   ...
   */
  private getMessageType(message: Message): 'text' | 'image' | 'audio' | 'document' | 'other' {
    if (message.hasMedia) {
   ...
   */
  private trimConversationHistory(session: UserSession): void {
