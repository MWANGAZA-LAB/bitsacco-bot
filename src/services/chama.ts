...
interface ChamaRules {
  ...
  votingThreshold: number; // percentage needed for decisions
...
class TypedChamaService {
  private chamaCache: Map<string, Chama> = new Map();
  private memberCache: Map<string, ChamaMember[]> = new Map();
  private votingCache: Map<string, any> = new Map();
  private contributions: Map<string, ChamaContribution[]> = new Map();
   ...
   */
  async createChama(adminUserId: string, adminPhoneNumber: string, chamaData: ChamaData): Promise<ChamaResult> {
   ...
   */
  async joinChama(chamaId: string, userId: string, phoneNumber: string, firstName: string): Promise<ChamaResult> {
    const startTime = performance.now();
   ...
   */
  async contributeToChama(chamaId: string, userId: string, amount: number): Promise<ContributionResult> {
    const startTime = performance.now();
   ...
   * Get user's chamas
   */
  async getUserChamas(userId: string): Promise<Chama[]> {
    try {
      const userChamas: Chama[] = [];
   ...
   * Update chama statistics
   */
  private updateChamaStats(chama: Chama): void {
   ...
   */
  private cleanupInactiveChamas(): void {
    const now = new Date();
    let cleanedCount = 0;
