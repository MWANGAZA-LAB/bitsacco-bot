...
interface SynthesisRequest {
  ...
  timestamp: Date;
  resolve: ((value: SynthesisResult) => void) | null;
...
class TypedVoiceSynthesizer {
  ...
  private fileCleanupAge: number = 24 * 60 * 60 * 1000; // 24 hours
   ...
   */
  async synthesizeText(text: string, options: SynthesisOptions = {}): Promise<SynthesisResult> {
    ...
    try {
      ...
      const voiceSettings: VoiceSettings = { ...this.defaultSettings, ...options };
  ...
  private async processQueue(): Promise<void> {
    if (!this.isEnabled || this.requestQueue.length === 0) return;
  ...
  private async processSynthesisRequest(request: SynthesisRequest): Promise<void> {
    ...
    try {
      ...
      const requestData: ElevenLabsRequestData = {
        text: request.text,
   ...
   */
  private async callElevenLabsAPI(requestData: ElevenLabsRequestData): Promise<Buffer> {
    try {
      if (!this.client) {
   ...
   */
  private async callElevenLabsVoicesAPI(): Promise<{ voices: any[] }> {
    try {
      if (!this.client) {
        throw new Error('ElevenLabs client not initialized');
      }
